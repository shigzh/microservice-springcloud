...
public class ConfigBean {
     ...
     * 下面这个设置既适用ribbon也适用于feign(feign是对ribbon的封装)
     *
     * 负载均衡有好几种实现策略，常见的有：
     * 1. 随机 (Random)
     * 2. 轮询 (RoundRobin)
    ...
    public IRule ribbonRule() {
        //return new BestAvailableRule();//最空闲连接策略;选择一个最小的并发请求的server
        return new RoundRobinRule();//轮询
        //return new RetryRule();//重试
        //return new RandomRule();//随机
        //return new WeightedResponseTimeRule();// 响应时间权重;根据响应时间分配一个weight，响应时间越长，weight越小，被选中的可能性越低。
    }
}
